...
const GSHEET_SNAPSHOT_ID = 'gsheet_live'
...
function makeLabel(sheetName, academicYear, semester) {
  ...
  return `GSheet: ${sheetName}${ay} (synced ${dateStr} ${timeStr})`
}
...
export default async function handler(req, res) {
  ...
  // Wipe previous gsheet_live entries and reinsert fresh — no new snapshot created
  await TimetableEntry.deleteMany({ dataset: GSHEET_SNAPSHOT_ID })
  const CHUNK = 1000
  let inserted = 0
  ...
  // Upsert a single snapshot record — same ID every time, just update the label/count
  await TimetableSnapshot.findOneAndUpdate(
    { snapshotId: GSHEET_SNAPSHOT_ID },
    {
      $set: {
        label,
  ...
  // Deactivate all other live snapshots so gsheet_live is the active one
  await TimetableSnapshot.updateMany(
    { type: 'live', snapshotId: { $ne: GSHEET_SNAPSHOT_ID } },
    { $set: { isActive: false } }
  )

  await GSheetConfig.findByIdAndUpdate('live', {
    lastSyncedAt: new Date(),
    lastSyncRows: inserted,
    lastSyncLabel: label,
  })

  return res.json({ success: true, inserted, label, warnings })
}
